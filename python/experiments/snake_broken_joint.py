...
python snake_broken_joint.py --trial_number=0
"""
...
path_to_output_snake = os.path.join(path_to_output, "snake_broken_joint/")
...
from scipy.optimize import minimize, Bounds
from snake_broken_joint_settings import return_snake_broken_joint_experiment_settings
...
def obj_and_grad_params(
    ...
    snake_length, broken_joint_ids, broken_joint_angles,
    ...
    Args:
        ...
        n_cp (int): Number of control points  for the motion trajectory.
    ...
    obj, grad_obj_g, grad_obj_pos_ = fun_obj_grad_g(g, pos_np_, gt, masses, a_weights, b_weights, fun_anisotropy_dir, w_fit_scaled, w_energy_scaled)
...
def fun_obj_grad_g(gs, pos_, gt, masses, a_weights, b_weights, fun_anisotropy_dir, w_fit_scaled, w_energy_scaled):
    '''
    Args:
...
class OptimizationBookkeeper:
    ...
    def callback(self, x):
        ...
        obj, grad_obj_g, grad_obj_pos_ = self.fun_obj_grad_g(g, pos_np_, self.gt, self.masses, self.a_weights, self.b_weights, self.fun_anisotropy_dir, self.w_fit_scaled, self.w_energy_scaled)
...
def main(_):
    ...
    settings_dict = return_snake_broken_joint_experiment_settings(trial_number)
    n_points_snake, n_angles, n_ts, n_cp = settings_dict["n_pts"], settings_dict["n_angles"], settings_dict["n_ts"], settings_dict["n_cp"]
    ...
    broken_joint_angles = broken_joint_angles.reshape(1, -1) * torch.ones(size=(n_ts, 1))
    ...
    save_path = os.path.join(path_to_output_snake, "snake_broken_joints_init{}.json".format(tag_experiment))
    ...
    save_path_opt = os.path.join(path_to_output_snake, "snake_broken_joints_opt{}.json".format(tag_experiment))
