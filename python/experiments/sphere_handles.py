"""This script can be executed to explore the motion of a sphere under diverse regularization energies using point handles.

In a terminal, with the conda environment turned on, run the following command line:

python sphere_handles.py --trial_number=0
"""
...
path_to_output_sphere = os.path.join(path_to_output, "sphere_handles/")
...
from sphere_handles_settings import return_sphere_handles_experiment_settings
from geometry_io import export_meshes_to_json
from geometry_generation import generate_sphere_mesh
from igl import cotmatrix, massmatrix, edges
...
from surface_energy import vmap_compute_mesh_vertex_normals, compute_mesh_vertex_normals, compute_mesh_surface_area, vmap_compute_voronoi_areas, compute_mesh_volume
from surface_shapes import factorize_laplacian, handle_deformations_generation_cubic_splines
from utils import print_quaternion, to_torch_sparse
...
def obj_and_grad(
    ...
    Luu, Quk, free_ids, handles_ids,
    faces_mesh, edges_mesh, volume_ref, edge_lengths_ref,
    force_0, torque_0, close_gait, gt,
    vertices_undeformed, vertices_ref_rot, handle_defos_init, free_handle_dofs,
    ...
    pos_ = handle_deformations_generation_cubic_splines(
        cps_tmp, Luu, Quk, free_ids, handles_ids, torch.tensor(vertices_undeformed), torch.tensor(vertices_ref_rot), handle_defos_init, free_handle_dofs, n_ts, close_motion=close_gait,
    )
...
def main(_):
    ...
    laplacian_torch = to_torch_sparse(laplacian).to_dense()
    mass_torch = to_torch_sparse(mass).to_dense()
    harmonic_order = 2
    ...
    handles, _ = generate_sphere_mesh(radius, 0)
    n_handles = handles.shape[0]
    ids = list(range(vertices.shape[0]))
    handles_ids = np.argmin(np.linalg.norm(handles.reshape(-1, 1, 3) - vertices.reshape(1, -1, 3), axis=-1), axis=1).tolist()
    handles = vertices[handles_ids]
    free_ids = np.setdiff1d(ids, np.array(handles_ids)).tolist()
    weights = np.ones(vertices.shape[0])
    Luu, Quk = factorize_laplacian(laplacian_torch, mass_torch, free_ids, handles_ids, harmonic_order=harmonic_order)
    
    ###########################################################################
    ## GET QUANTITIES RELATED TO THE TRIAL NUMBER
    ###########################################################################
    
    setting_dict = return_sphere_handles_experiment_settings(FLAGS.trial_number)
    ...
    handle_defos_init = torch.zeros(size=(n_ts, n_handles, 3))
    fixed_handle_dofs = []
    free_handle_dofs = np.setdiff1d(list(range(3*n_handles)), fixed_handle_dofs).tolist()

    n_free_dofs = len(free_handle_dofs)
    ...
    pos_ = handle_deformations_generation_cubic_splines(
        cps, Luu, Quk, free_ids, handles_ids, torch.tensor(vertices), torch.tensor(vertices_perturbed), handle_defos_init, free_handle_dofs, n_ts, close_motion=close_gait,
    )
    ...
    save_path = os.path.join(path_to_output_sphere, "sphere_handles_init_{:02d}.json".format(FLAGS.trial_number))
    ...
    obj_and_grad_scipy = lambda x: obj_and_grad(
        ...
        force_0, torque_0, close_gait, gt,
        vertices, vertices_perturbed, handle_defos_init, free_handle_dofs,
        w_fit_scaled, w_vol_scaled, w_edges_scaled,
        fun_anisotropy_dir, fun_obj_grad_g,
    )
    ...
    pos_opt_ = handle_deformations_generation_cubic_splines(
        cps_opt, Luu, Quk, free_ids, handles_ids, torch.tensor(vertices), torch.tensor(vertices_perturbed), handle_defos_init, free_handle_dofs, n_ts, close_motion=close_gait,
    )
    ...
    save_path = os.path.join(path_to_output_sphere, "sphere_handles_opt_{:02d}.json".format(FLAGS.trial_number))
