"""This script can be executed to recover the optimal gait of a purcell swimmer in a viscous fluid.

In a terminal, with the conda environment turned on, run the following command line:

python purcell.py --trial_number=0
"""
...
path_to_output_purcell = os.path.join(path_to_output, "purcell/")

if not os.path.exists(path_to_output_purcell):
    os.makedirs(path_to_output_purcell)
...
from scipy.optimize import minimize, Bounds
from purcell_settings import return_purcell_experiment_settings
from purcell_shapes import purcell_generator_polar_fixed_angles_cubic_splines_origin_symmetry, generate_control_points, purcell_generate_fun_anisotropy_dir
...
def obj_and_grad_params(
    ...
    force_0, torque_0, n_pts_per_segment, n_points, close_gait,
    ...
    Args:
        ...
        n_pts_per_segment (float): Number of points per segment of the object.
    ...
    pos_ = purcell_generator_polar_fixed_angles_cubic_splines_origin_symmetry(
        params, n_steps, n_pts_per_segment, e1bye2=e1bye2, e3bye2=e3bye2, total_length=total_length, close_gait=close_gait,
    )
...
class OptimizationBookkeeper:
    ...
    def callback(self, x):
        ...
        pos_ = purcell_generator_polar_fixed_angles_cubic_splines_origin_symmetry(
            params, self.n_steps, self.n_pts_per_segment, e1bye2=self.e1bye2, e3bye2=self.e3bye2, total_length=self.total_length, close_gait=self.close_gait
        )
...
def main(_):
    ...
    settings_dict = return_purcell_experiment_settings(trial_number)
    n_pts_per_segment, n_cp, n_ts = settings_dict["n_pts_per_segment"], settings_dict["n_cp"], settings_dict["n_ts"]
    n_hinges = 4
    n_points = n_hinges + 3 * n_pts_per_segment
    ...
    fun_anisotropy_dir = purcell_generate_fun_anisotropy_dir(n_pts_per_segment)
    
    ###########################################################################
    ## GENERATE THE GEOMETRY
    ###########################################################################

    radii = 1.0 * torch.ones(n_cp)
    torch.manual_seed(0)
    params = radii + init_perturb_magnitude * torch.randn_like(radii)
    ...
    save_path = os.path.join(path_to_output_purcell, "purcell_init.json")
    ...
    pos_opt_ = purcell_generator_polar_fixed_angles_cubic_splines_origin_symmetry(
        params_opt, n_ts, n_pts_per_segment, e1bye2=e1bye2, e3bye2=e3bye2, total_length=total_length, close_gait=close_gait
    ).numpy()
    ...
    save_path_opt = os.path.join(path_to_output_purcell, "purcell_opt{}.json".format(tag_experiment))
