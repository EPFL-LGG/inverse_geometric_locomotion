"""This script can be executed to explore the motion of a sphere under diverse regularization energies using laplacian eigenmodes.

In a terminal, with the conda environment turned on, run the following command line:

python sphere_laplacian.py --trial_number=0
"""
...
path_to_output_sphere = os.path.join(path_to_output, "sphere_laplacian/")
...
from sphere_laplacian_settings import return_sphere_laplacian_experiment_settings
...
from surface_shapes import laplacian_modes_generation_cubic_splines
...
def obj_and_grad(
    ...
    vertices_undeformed, vertices_ref_rot, modes_torch,
    ...
    cps_tmp = params_torch.reshape(n_cp, -1, 3)
    n_modes = cps_tmp.shape[1]
    pos_ = laplacian_modes_generation_cubic_splines(cps_tmp, modes_torch, vertices_undeformed, n_ts, close_motion=close_gait, vertices_ref_rot=vertices_ref_rot)
    ...
    grad_params_obj[:3*n_modes] = 0.0 # First shape is fixed
...
def main(_):
    ...
    setting_dict = return_sphere_laplacian_experiment_settings(FLAGS.trial_number)
    n_ts, n_cp, n_modes = setting_dict['n_ts'], setting_dict['n_cp'], setting_dict['n_cp'] - 1
    ...
    ###########################################################################
    ## DEFINE THE EIGENMODES
    ###########################################################################

    # +1 since we remove the rigid translation mode
    lambdas, modes = spsl.eigsh(-laplacian, n_modes+1, M=mass, sigma=-1e-15, which='LM')
    modes = modes[:, 1:]
    lambdas = lambdas[1:]
    modes_torch = torch.tensor(modes)
    ...
    cps = torch.zeros(n_cp, n_modes, 3)
    torch.manual_seed(0)
    cps += init_perturb_magnitude * torch.randn(n_cp, n_modes, 3)
    cps[0] = 0.0

    pos_ = laplacian_modes_generation_cubic_splines(cps, modes_torch, vertices_ref, n_ts, close_motion=close_gait, vertices_ref_rot=torch.tensor(vertices_perturbed))
    ...
    save_path = os.path.join(path_to_output_sphere, "sphere_laplacian_init_{:02d}.json".format(FLAGS.trial_number))
    ...
    obj_and_grad_scipy = lambda x: obj_and_grad(
        ...
        force_0, torque_0, close_gait, gt,
        torch.tensor(vertices), torch.tensor(vertices_perturbed), modes_torch,
        w_fit_scaled, w_vol_scaled, w_edges_scaled,
        fun_anisotropy_dir, fun_obj_grad_g,
    )
    
    params_0 = cps.reshape(-1,).numpy()
    lb = -3.0 * np.ones(shape=(n_cp*3*n_modes,))
    ub = 3.0 * np.ones(shape=(n_cp*3*n_modes,))
    ...
    cps_opt = params_torch_opt.reshape(n_cp, -1, 3)

    pos_opt_ = laplacian_modes_generation_cubic_splines(cps_opt, modes_torch, vertices_ref, n_ts, close_motion=close_gait, vertices_ref_rot=torch.tensor(vertices_perturbed))
    ...
    save_path = os.path.join(path_to_output_sphere, "sphere_laplacian_opt_{:02d}.json".format(FLAGS.trial_number))
