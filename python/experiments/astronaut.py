"""This script can be executed to have an armadilloy astronaut rotate.

In a terminal, with the conda environment turned on, run the following command line:

python astronaut.py --trial_number=0
"""
...
path_to_output_astronaut = os.path.join(path_to_output, "astronaut_all_vertices/")

if not os.path.exists(path_to_output_astronaut):
    os.makedirs(path_to_output_astronaut)
...
from scipy.optimize import minimize, Bounds
from astronaut_settings import return_astronaut_experiment_settings
...
FLAGS = flags.FLAGS
flags.DEFINE_integer("trial_number", None, "The trial number for the experiment.", lower_bound=0, upper_bound=2, required=True)
...
def obj_and_grad_params(
    ...
    pos_target_, edges_graph, edge_lengths_ref, length_threshold, n_edge_disc,
    w_fit_scaled, w_energy_scaled, w_edges_scaled, w_edges_repulsion_scaled, w_gait_fit_scaled,
    ...
    Args:
        ...
        length_threshold (float): Length threshold for the edge repulsion term.
        n_edge_disc (int): Number of discretization points for the edge repulsion term.
    ...
    obj, grad_obj_g, grad_obj_pos_ = fun_obj_grad_g(g, pos_np_, gt, pos_target_, masses, a_weights, b_weights, edges_graph, edge_lengths_ref, length_threshold, n_edge_disc, w_fit_scaled, w_energy_scaled, w_edges_scaled, w_edges_repulsion_scaled, w_gait_fit_scaled, fun_anisotropy_dir)
...
def fun_obj_grad_g(gs, pos_, gt, pos_target_, masses, a_weights, b_weights, edges_graph, edge_lengths_ref, length_threshold, n_edge_disc, w_fit_scaled, w_energy_scaled, w_edges_scaled, w_edges_repulsion_scaled, w_gait_fit_scaled, fun_anisotropy_dir):
    '''
    Args:
    ...
    if w_edges_repulsion_scaled == 0.0:
        obj_edge_repulsion, grad_edge_repulsion_g, grad_edge_repulsion_pos_ = 0.0, 0.0, 0.0
    else:
        obj_edge_repulsion = edge_centers_repulsion(pos_, edges_graph, length_threshold, n_edge_disc)
        grad_edge_repulsion_g, grad_edge_repulsion_pos_ = grad_edge_centers_repulsion(pos_, edges_graph, length_threshold, n_edge_disc)
    ...
    obj = w_fit_scaled * obj_fit + w_energy_scaled * obj_energy + w_edges_scaled * obj_edges + w_edges_repulsion_scaled * obj_edge_repulsion + w_gait_fit_scaled * obj_gait
    grad_g = w_fit_scaled * grad_fit_g + w_energy_scaled * grad_energy_g + w_edges_scaled * grad_edges_g + w_edges_repulsion_scaled * grad_edge_repulsion_g + w_gait_fit_scaled * grad_gait_g
    grad_pos_ =  w_fit_scaled * grad_fit_pos_ + w_energy_scaled * grad_energy_pos_ + w_edges_scaled * grad_edges_pos_ + w_edges_repulsion_scaled * grad_edge_repulsion_pos_ + w_gait_fit_scaled * grad_gait_pos_
...
class OptimizationBookkeeper:
    def __init__(
        ...
        self.w_edges_repulsion_scaled = w_edges_repulsion_scaled
    ...
    def callback(self, x):
        ...
        obj, grad_obj_g, grad_obj_pos_ = fun_obj_grad_g(g, pos_np_, self.gt, self.pos_target_, self.masses, self.a_weights, self.b_weights, self.edges_graph, self.edge_lengths_ref, self.length_threshold, self.n_edge_disc, self.w_fit_scaled, self.w_energy_scaled, self.w_edges_scaled, self.w_edges_repulsion_scaled, self.w_gait_fit_scaled, self.fun_anisotropy_dir)
...
def main(_):
    ...
    settings_dict = return_astronaut_experiment_settings(trial_number)
    ...
    w_fit, w_energy, w_edges, w_edges_repulsion, w_gait_fit = settings_dict['w_fit'], settings_dict['w_energy'], settings_dict['w_edges'], settings_dict['w_edges_repulsion'], settings_dict['w_gait_fit']
    gt = np.array(settings_dict['gt'])
    repulsion_length_threshold = settings_dict['repulsion_length_threshold']
    n_edge_disc = settings_dict['n_edge_disc']
    ...
    total_mass = rho * n_points
    w_fit_scaled = w_fit / (aabb_diag ** 2)
    w_energy_scaled = w_energy / (total_mass * torch.sum(initial_edge_lengths).item()  ** 2)
    w_edges_scaled = w_edges / (torch.mean(initial_edge_lengths).item() ** 2)
    w_edges_repulsion_scaled = w_edges_repulsion / (torch.mean(initial_edge_lengths).item() ** 2)
    ...
    normals_ = fun_anisotropy_dir(pos_).numpy()
    ...
    save_path = os.path.join(path_to_output_astronaut, "astronaut_init{}.json".format(tag_experiment))
    ...
    obj_and_grad_scipy = lambda x: obj_and_grad_params(
        ...
        pos_ref_, edges_graph, initial_edge_lengths, repulsion_length_threshold, n_edge_disc,
        w_fit_scaled, w_energy_scaled, w_edges_scaled, w_edges_repulsion_scaled, w_gait_fit_scaled,
        fun_anisotropy_dir, fun_obj_grad_g,
    )
    ...
    save_path_opt = os.path.join(path_to_output_astronaut, "astronaut_opt{}.json".format(tag_experiment))
    ...
    weights_optim = {
        ...
        'w_edges_repulsion': w_edges_repulsion,
        'w_edges_repulsion_scaled': w_edges_repulsion_scaled,
