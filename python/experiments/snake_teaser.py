"""This script can be executed to guide a snake through obstacles.

In a terminal, with the conda environment turned on, run the following command line:

python snake_teaser.py --trial_number=0
"""
...
path_to_output_snake = os.path.join(path_to_output, "snake_teaser/")
...
def main(_):
    ...
    save_path = os.path.join(path_to_output_snake, "snake_teaser_init{}.json".format(tag_experiment))
    ...
    save_path_opt = os.path.join(path_to_output_snake, "snake_teaser_opt{}.json".format(tag_experiment))
    ...
    print_quaternion(torch.tensor(gt[-1, :4]))
